...
import os
# sys.path.append(str(os.getcwd()) + '//..//util//')
import max_euclidean
from scipy.special import comb


try:
    conn = psycopg2.connect("dbname='hurricane' user='' host='localhost' password=''")
except:
...
def get_list_hurricane_tid(table_name):
...
def confirm_neighbor(top_k, length_k_sub_trajectory, list_potential_neighbor, distance_threshold):
    ...
    for potential_neighbor in list_potential_neighbor:
        # top_k_max, max_distance = max_euclidean.calculate_top_k(len(length_k_sub_trajectory), length_k_sub_trajectory, potential_neighbor[1])
...
def stat_dsm(trajectory_table, point_table, candidate_table, original_list_label, list_permuted_dataset, list_min_p, list_hurricane_tid, parameter):
    ...
    for trajectory_tid in list_hurricane_tid:
...
def main():
    ...
    candidate_table = 'candidates_test'
    ...
    list_hurricane_tid = get_list_hurricane_tid(trajectory_table)
    list_hurricane_tid = np.array(list_hurricane_tid)
    list_hurricane_tid = list_hurricane_tid[:, 0].tolist()
    ...
    delta = stat_dsm(trajectory_table, point_table, candidate_table, original_list_label, list_permuted_dataset, list_min_p, list_hurricane_tid, parameter)
